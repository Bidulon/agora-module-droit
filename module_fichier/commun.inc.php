...
////	TAILLE D'UN FICHIER (date: "toutes"/"2008-12-01 12:00:00";)
////
function taille_fichier($infos_fichier, $version="toutes")
{
	global $objet;

	// Infos sur le fichier et sur la/les versions du fichier
	$infos_version_fichier = infos_version_fichier($infos_fichier["id_fichier"],$version);
	$chemin_dossier = PATH_MOD_FICHIER.chemin($objet["fichier_dossier"],$infos_fichier["id_dossier"],"url");

	// Taille de toutes les versions du fichier  OU  d'une version précise du fichier
	if($version!="toutes")	{ return filesize($chemin_dossier.$infos_version_fichier["nom_reel"]); }
	else
	{
		$taille_fichier = 0;
		foreach($infos_version_fichier as $version_fichier)		{ $taille_fichier += filesize($chemin_dossier.$version_fichier["nom_reel"]); }
...
function taille_dossier_bdd($id_dossier, $acces="lecture")
{
	...
	// Liste l'arborescence du dossier (en ajoutant le dossier courant)
	$liste_dossiers_arborescence = arborescence($objet["fichier_dossier"], $id_dossier, $acces);
	$liste_dossiers_arborescence[] = array("id_dossier" => $id_dossier);
	...
	foreach($liste_dossiers_arborescence as $infos_dossier)
	{
		...
		foreach($liste_fichiers as $infos_fichier)		{ $taille_dossier += taille_fichier($infos_fichier); }
	}
