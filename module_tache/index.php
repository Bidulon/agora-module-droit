...
<table id="contenu_principal_table"><tr>
	...
	<td>
		...
		$liste_taches = db_tableau("SELECT * FROM gt_tache WHERE id_dossier='".intval($_GET["id_dossier"])."' ".sql_affichage($objet["tache"],$_GET["id_dossier"])." ".tri_sql($objet["tache"]["tri"])." ");
		foreach($liste_taches as $tache_tmp)
		{
			...
			if($tache_tmp["priorite"]!="")	$tache_tmp["priorite"] = "<img src=\"".PATH_TPL."module_tache/priorite_".$tache_tmp["priorite"].".png\" ".infobulle($trad["TACHE_priorite"]." ".@$trad["TACHE_priorite_".$tache_tmp["priorite"]])." /> &nbsp; ";
			////	RESPONSABLES
			...
			require PATH_INC."element_menu.inc.php";
				...
				if($_REQUEST["type_affichage"]=="bloc")
				{
					...
					echo "<div style='position:absolute;right:0px;bottom:0px;font-weight:bold;cursor:help;'>";
						...
						if($responsables_infobulle!="")										echo "<img src=\"".PATH_TPL."divers/utilisateurs_small.png\" ".$responsables_infobulle." /> &nbsp;";
					echo "</div>";
					////	TITRE + DESCRIPTION
				...
				else
				{
					echo "<table class='div_elem_table'><tr>";
						...
						echo "<td class='div_elem_td div_elem_td_right'>";
							if($responsables_infobulle!="")	echo "<span ".$responsables_infobulle."><img src=\"".PATH_TPL."divers/utilisateurs_small.png\" /> ".$responsables_text."</span><img src=\"".PATH_TPL."divers/separateur.gif\" />";
