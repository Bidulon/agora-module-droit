...
////	AFFICHAGE D'UNE BARRE DE STATUT
////
function status_bar($pourcent, $txt_barre, $txt_infobulle, $couleur_pourcent="jaune", $width_barre="120px")
{
	// init
	$bar_tmp = "";
	if($pourcent==0)	$pourcent = 0.5;
	...
	// Affichage
	$bar_tmp = "<table style=\"display:".$display.";width:".$width_barre.";height:18px;vertical-align:middle;background-image:url(".PATH_TPL."divers/barre_background.png);\" cellpadding=\"0\" cellspacing=\"0\"><tr>";
		$bar_tmp .= "<td style=\"width:".$pourcent."%;text-align:left;background-image:url(".PATH_TPL."divers/barre_".$couleur_pourcent.".png);\">";
			$bar_tmp .= "<span style=\"position:absolute;width:".$width_barre.";line-height:18px;padding:0px;margin:0px;font-size:9px;font-weight:normal;color:#000;text-align:center;cursor:help;\" ".infobulle($txt_infobulle).">".$txt_barre."</span>";
		$bar_tmp .= "<img src=\"".PATH_TPL."divers/vide.png\" /></td>";
		$bar_tmp .= "<td><img src=\"".PATH_TPL."divers/vide.png\" /></td>";
	$bar_tmp .= "</tr></table>";
	return $bar_tmp;
}
...
function tache_debut_fin($tache_tmp, $aff_text_infobulle=false)
{
	if($tache_tmp["date_debut"]!="" || $tache_tmp["date_fin"]!="")
	{
		...
		elseif($tache_tmp["date_debut"]!="" && $tache_tmp["date_fin"]!="" && substr($tache_tmp["date_debut"],0,10)!=substr($tache_tmp["date_fin"],0,10))
		{
			...
			// Resultat
			return status_bar($pourcent, $txt_barre, $txt_infobulle, $couleur_pourcent, "150px")." &nbsp; &nbsp;";
	 	}
	 	////	Affichage texte
