			...
			<tr>
				<td>
					<div style="height:200px;overflow-y:auto;">
						...
						echo "<div class=\"titre_espace\">".$espace_tmp["nom"]."</div>";
						if($espace_tmp["description"]!="")	echo "<div>".$espace_tmp["description"]."</div>";
						foreach(modules_espace($espace_tmp["id_espace"]) as $module_tmp){
							echo "<img src=\"".PATH_TPL.$module_tmp["module_dossier_fichier"]."/menu.png\" ".infobulle($trad[strtoupper($module_tmp["nom"])."_description_module"])." style=\"height:28px;margin-top:10px;margin-right:5px;cursor:help;\" /> ";
						}

						////	DROITS D'ACCES
						echo "<hr /><div class=\"titre_espace_bis\"><i>".$trad["EDIT_OBJET_droit_acces"]." :</i></div>";
						// ESPACE PUBLIC
						if(db_valeur("SELECT count(*) FROM gt_jointure_espace_utilisateur WHERE id_espace='".$espace_tmp["id_espace"]."' AND invites=1") > 0)
							echo "<div class=\"affectation_espace\"><img src=\"".PATH_TPL."divers/planete.png\" style=\"height:15px;\" /> <b>".$trad["espace_public"]."</b></div>";
						...
						if($tous_users_site > 0)
							echo "<div class=\"affectation_espace\"><img src=\"".PATH_TPL."divers/utilisateurs_small.png\" style=\"height:15px;\" /> <b>".$trad["ESPACES_tous_utilisateurs"]."</b></div>";
						...
						foreach(users_espace($espace_tmp["id_espace"],"tableau") as $user_tmp)
						{
							...
							echo "<div class=\"lien affectation_espace\" ".popup_user($user_tmp["id_utilisateur"])."><img src=\"".PATH_TPL."divers/".$icone_statut_user.".png\" style='height:15px;' /> ".$user_tmp["prenom"]." ".$user_tmp["nom"]."</div>";
						}
						// Définir droits accès
				...
				<td style="width:30px;text-align:right;vertical-align:top;cursor:pointer;">
					<?php
					////	GESTION DES ACCES / MODIFIER
					...
					////	SUPPRIMER
					if($espace_tmp["id_espace"]==$_SESSION["espace"]["id_espace"])	$action_onclick = "alert('".addslashes($trad["MSG_ALERTE_suppr_espace_impossible"])."');";
					else															$action_onclick = "confirmer('".addslashes($trad["ESPACES_confirm_suppr_espace"])."','index.php?action=espace_suppr&id_espace=".$espace_tmp["id_espace"]."');";
					echo "<img src=\"".PATH_TPL."divers/supprimer.png\" style=\"margin-top:5px;\" onclick=\"".$action_onclick."\" ".infobulle("<span style=\'color:#ff0000;\'>".$trad["ESPACES_supprimer_espace"]."</span>")." />";
					?>
				</td>
			</tr>
		</table>
