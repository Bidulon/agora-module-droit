...
<table id="contenu_principal_table"><tr>
	<td id="menu_gauche_block_td" class="noprint">
		<div id="menu_gauche_block_flottant">
			...
			if(count($AGENDAS_AFFICHES)>0)
			{
				...
				////
				echo "<form action='index.php' method='get' class='menu_gauche_block content'>";
					foreach($AGENDAS_AFFICHES as $cle_tmp => $agenda_tmp)
					{
						...
						echo "<div class='tr_survol' ".infobulle($agenda_tmp["description"]).">";
							echo "<input type='checkbox' name='agendas_demandes[]' value='".$agenda_tmp["id_agenda"]."' id='".$agenda_tmp["id_agenda"]."' ".$check_tmp." />";
					...
					if($_SESSION["user"]["admin_general"]==1)
					{
						...
						echo "<a href=\"javascript:redir('index.php?afficher_tous_agendas=".$tous_get."');\" style='float:left;margin-top:10px' ".infobulle($trad["admin_only"]).">".$tous_libelle."</a>";
					}
					////	Bouton de validation
					echo "<input type='submit' value=\"".$trad["afficher"]."\" class='button_big' style='width:80px' />";
					echo "</div>";
				echo "</form>";
	...
	<td>
		...
		////	TABLEAU DE SYNTHESE DES AGENDAS
		////
		require "agendas.inc.php";
				...
				echo "<table style='width:100%;cursor:pointer;font-weight:bold;' cellpadding='4px'><tr>";
					...
					echo "<td class='print_titre libelle_agenda'>";
						...
						if(preg_match("/semaine|jour/i",$_SESSION["cfg"]["espace"]["agenda_affichage"]))
						{
							...
							echo " &nbsp; (".$trad["AGENDA_num_semaine"]." ".strftime(" %W",$config["agenda_debut"]).")";
						}
		...
		////	AUCUN AGENDA
		if(count($_SESSION["cfg"]["espace"]["agendas_affiches"])==0)	echo "<div class='div_elem_aucun'>".$trad["AGENDA_aucun_agenda_visible"]."</div>";
		?>
	</td>
</tr></table>
