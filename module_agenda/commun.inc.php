...
////
function liste_evenements($id_agenda, $T_debut_periode, $T_fin_periode, $type_sortie="lecture")
{
	...
	$liste_evenements_tmp = db_tableau("SELECT  T1.*  FROM gt_agenda_evenement T1, gt_agenda_jointure_evenement T2  WHERE  T1.id_evenement=T2.id_evenement  AND  T2.id_agenda='".intval($id_agenda).")'  AND  T2.confirme='1'  AND  (".$sql_periode." OR ".$periodicite.")  ORDER BY T1.date_debut asc");
...
////	SUPPRESSION D'UN AGENDA DE RESSOURCE (SI ACCES TOTAL) & DES ÉVÉNEMENTS LIÉS ! UNIQUEMENT ! A CET AGENDA
////
function suppr_agenda($id_agenda)
{
	global $AGENDAS_AFFICHES;
	if($AGENDAS_AFFICHES[$id_agenda]["droit"]==3)
	{
		global $objet;
