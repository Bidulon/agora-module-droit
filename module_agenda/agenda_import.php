...
if(isset($_POST["champs_evenement"]) && $_POST["evenement_import"])
{
	...
	{
		db_query("INSERT INTO gt_agenda_evenement SET titre=".db_format(@$evenement_tmp["titre"],"slash").", description=".db_format(@$evenement_tmp["description"],"slash").", date_debut=".db_format(@$evenement_tmp["date_debut"]).", date_fin=".db_format(@$evenement_tmp["date_fin"]).", id_utilisateur='".$_SESSION["user"]["id_utilisateur"]."', date_crea='".db_insert_date()."'");
	...
	echo "<form action=\"".php_self()."\" method=\"post\" OnSubmit=\"return controle_evenements();\">";
		echo "<table cellpadding=\"5px\">";
			...
			echo "<tr style=\"background-color:#aaa\">";
				...
				echo "<td class='lien' onClick=\"selection_import();\" ".infobulle($trad["inverser_selection"])."><img src=\"".PATH_TPL."divers/tri.png\" /></td>";
