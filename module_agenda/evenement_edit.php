...
if(@$_REQUEST["id_evenement"]<1){
	...
	if(!empty($_GET["select_heure_debut"]) && $_GET["select_heure_debut"]!="null"){
		$evt_tmp["date_debut"] = strtotime(strftime("%Y-%m-%d ".num2carac($_GET["select_heure_debut"]).":00", $_GET["date"]));
		$evt_tmp["date_fin"] = strtotime(strftime("%Y-%m-%d ".num2carac($_GET["select_heure_fin"]).":00", $_GET["date"])) + 3600;
	}
}
else {
...
function afficher_periodicite()
{
	...
	if(periodicite_type=="")	{ afficher_dynamic('div_periodicite',null,false);	afficher('div_periodicite_options',false); }
	else						{ afficher_dynamic('div_periodicite',null,true);	afficher('div_periodicite_options',true); }
...
function controle_creneaux_horaires()
{
	...
	{
		recup_dates();
		requete_ajax("GET", "evenement_edit_verif.php?id_evenement=<?php echo @$evt_tmp["id_evenement"]; ?>&datetime_debut="+datetime_debut+"&datetime_fin="+datetime_fin+agendas_selectionnes);
	...
	else if(element("message_creneaux_horaires").style.display=="none")		afficher_dynamic("message_creneaux_horaires",null,true);
}
...
<form action="<?php echo php_self(); ?>" method="post" enctype="multipart/form-data" style="padding:20px;font-weight:bold;" OnSubmit="return controle_formulaire();">
	...
	echo "<fieldset style='text-align:center;'>";
		...
		if(@$evt_tmp["description"]=="" && $_SESSION["user"]["id_utilisateur"]>0)	echo "<script type='text/javascript'>  masquer_block_description_onload();  </script>";
	echo "</fieldset>";
			...
			echo "<div id='div_periodicite'>";
				...
				echo "<div id='div_periodicite_options' class='div_periodicite'><table style='width:100%;margin-top:10px;'><tr>";
					...
					echo "<td style='text-align:left;padding-left:15px;vertical-align:middle;'>";
						$evt_tmp["period_date_exception"] = text2tab(@$evt_tmp["period_date_exception"]);
						$cpt_depart = (count($evt_tmp["period_date_exception"])>0)  ?  count($evt_tmp["period_date_exception"])  :  0;
						echo "<script> var cpt_excep = ".$cpt_depart."; </script>";
						echo $trad["AGENDA_exception_periodicite"]." &nbsp;<img src=\"".PATH_TPL."divers/plus2.png\" class='lien' onclick=\"if(cpt_excep<10) {afficher('span_period_exception'+cpt_excep,true,'block'); cpt_excep ++;}\" ".infobulle($trad["ajouter"])." />";
						for($cpt=0; $cpt<10; $cpt++)
						{
							// Prépare la date d'exception et l'id du champ
							if(@$evt_tmp["period_date_exception"][$cpt]!="")	{ $date_excep_input = $evt_tmp["period_date_exception"][$cpt];   $date_excep_cal = strtotime($evt_tmp["period_date_exception"][$cpt]); }
							else												{ $date_excep_input = "";   $date_excep_cal = time(); }
							$id_input = "period_exception".$cpt;
							$init_src_iframe = " if(element('calendrier_".$id_input."').src=='')  element('calendrier_".$id_input."').src='".PATH_INC."calendrier.inc.php?date_affiche=".$date_excep_cal."&date_selection=".$date_excep_cal."&champ_modif=".$id_input."'; ";
							// Affiche l'exception s'il y en a
							echo "<div id='span_".$id_input."' style='display:block;padding:3px;".( ($date_excep_input=="")?"display:none;":"" )."'>";
								echo "<iframe id='calendrier_".$id_input."' class='menu_context calendrier_flottant' scrolling='auto'></iframe>";
								echo "<input type='text' name='period_date_exception[]' id='".$id_input."' value=\"".$date_excep_input."\" class='calendrier_input' onClick=\"afficher_dynamic('calendrier_".$id_input."'); ".$init_src_iframe."\" readonly /> ";
								echo "<img src=\"".PATH_TPL."divers/supprimer.png\" class='lien' height='13px' onclick=\"set_value('".$id_input."',''); afficher('span_".$id_input."',false); cpt_excep --;\" ".infobulle($trad["supprimer"])." /> ";
							echo "</div>";
						}
					echo "</td>";
				echo "</tr></table></div>";
		...
		echo "<div style='max-height:240px;overflow:auto;padding-left:20px;'>";
			...
			foreach($groupes_users as $groupe_tmp)
			{
				...
				echo "<div class='affect_users_groupes lien' ".infobulle(str_replace(",","<br>",$groupe_tmp["users_title"])).">";
					echo "<img src=\"".PATH_TPL."divers/utilisateurs_groupe.png\" class='icone_groupe' />";
