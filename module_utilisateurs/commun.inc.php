	...
	if($id_utilisateur > 0)
	{
		...
		if($_SESSION["user"]["admin_general"]==1 && $action=="suppression")
		{
			...
			else
			{
				...
				@unlink(PATH_MOD_USER.db_valeur("SELECT photo FROM gt_utilisateur WHERE id_utilisateur=".db_format($id_utilisateur)));
				...
				db_query("DELETE FROM gt_utilisateur_adresse_ip			WHERE id_utilisateur=".db_format($id_utilisateur));
