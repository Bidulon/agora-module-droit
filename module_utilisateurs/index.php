			...
			<div class="menu_gauche_block content">
				...
				if($_SESSION["user"]["admin_general"]==1 && ($affichage_users=="site" || count(espaces_affectes_user())>1))
				{
					...
					echo "<div class='menu_gauche_ligne'>";
						echo "<div class='menu_gauche_img'><img src=\"".PATH_TPL."divers/utilisateurs.png\" /></div>";
				...
				if($_SESSION["espace"]["droit_acces"]==2)
				{
					...
					echo "<div class='menu_gauche_ligne lien' onclick=\"popup('".PATH_DIVERS."contact_import.php?type_import=utilisateurs');\" ".infobulle($trad["contact_import_formats"])."><div class='menu_gauche_img'><img src=\"".PATH_TPL."divers/import.png\" /></div><div class='menu_gauche_txt'>".$trad["users_importer"]."</div></div>";
				}
				////	EXPORTER DES CONTACTS
				if($_SESSION["user"]["id_utilisateur"]>0)
					echo "<div class='menu_gauche_ligne lien' onclick=\"popup('".PATH_DIVERS."contact_export.php?type_export=utilisateurs');\" ".infobulle($trad["contact_export_formats"])."><div class='menu_gauche_img'><img src=\"".PATH_TPL."divers/export.png\" /></div><div class='menu_gauche_txt'>".$trad["users_exporter"]."</div></div>";
				////	ENVOI DES COORDONNEES DE CONNEXION PAR MAIL (REGENERE LE PASSWORD !!)
				if($_SESSION["user"]["admin_general"]==1)
					echo "<div class='menu_gauche_ligne lien' onclick=\"popup('envoi_coordonnees.php');\" ".infobulle($trad["UTILISATEURS_envoi_coordonnees_infos"])."><div class='menu_gauche_img'><img src=\"".PATH_TPL."divers/envoi_coordonnees.png\" /></div><div class='menu_gauche_txt'>".$trad["UTILISATEURS_envoi_coordonnees"]."</div></div>";
				////	ENVOI INVITATION PAR MAIL
				if(@$_SESSION["user"]["envoi_invitation"]=="1")
					echo "<div class='menu_gauche_ligne lien' onclick=\"popup('invitation.php');\" ".infobulle($trad["UTILISATEURS_envoi_invitation_info"])."><div class='menu_gauche_img'><img src=\"".PATH_TPL."divers/acces_utilisateur_plus.png\" /></div><div class='menu_gauche_txt'>".$trad["UTILISATEURS_envoi_invitation"]."</div></div>";
				////	GESTION DES GROUPES D'UTILISATEUR
				if(droit_ajout_groupe()==true)
				{
					echo "<hr />";
					echo "<div class='menu_gauche_ligne'>";
						echo "<div class='menu_gauche_img'><img src=\"".PATH_TPL."divers/utilisateurs_groupe.png\" /></div>";
						echo "<div class='menu_gauche_txt'>";
		...
		foreach(tableau_elements_page($users_total) as $user_tmp)
		{
			...
			$id_div_user = div_element($objet["utilisateur"], $user_tmp["id_utilisateur"]);
				...
				$id_menu_contextuel = "menu_context_user_".$user_tmp["id_utilisateur"];
					...
					echo "</div>";
					echo "<img src=\"".PATH_TPL."divers/options.png\" style='height:26px;' id='icone_".$id_menu_contextuel."' onMouseOver=\"afficher_menu_contextuel('".$id_menu_contextuel."');\" onMouseOut=\"afficher('".$id_menu_contextuel."',false);\" />";
				echo "</div>";
				...
				$img_statut_user = (preg_match("/admin/i",$statut_user))  ?  "&nbsp; <img src=\"".PATH_TPL."divers/acces_".$statut_user.".png\" style='cursor:help;height:20px;' ".infobulle($trad["UTILISATEURS_".$statut_user])." />"  :  "";
