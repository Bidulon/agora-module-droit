...
////	INITIALISATIONS
////
define("VERSION_AGORA", "2.13.2.4");
define("VERSION_MAJ_DATE", 20120715);
...
if(version_compare(@$_SESSION["agora"]["version_agora"],VERSION_AGORA,"<")  ||  @$_SESSION["agora"]["mise_a_jour"] < VERSION_MAJ_DATE)
{
	...
	else
	{
		...
		////	AJOUTE LE CHAMPS "envoi_invitation" DANS LA TABLE "gt_jointure_espace_utilisateur"
		db_maj_champ_ajoute("2.11.0", "gt_jointure_espace_utilisateur", "envoi_invitation", "ALTER TABLE gt_jointure_espace_utilisateur ADD envoi_invitation tinyint unsigned DEFAULT NULL AFTER droit");
		...
		db_maj_champ_rename("2.11.0", "gt_tache", "important", "ALTER TABLE gt_tache CHANGE `important` `priorite` TINYTEXT");
		...
		db_maj_query("2.11.2", "ALTER TABLE gt_fichier CHANGE vignette vignette TINYTEXT NULL", false);
		...
		if($champ_existe==false)
		{
			foreach(db_colonne("SELECT id_fichier FROM gt_fichier_version WHERE nom is null") as $id_fichier_tmp){
				...
				db_query("UPDATE gt_fichier_version SET nom=".db_format($nom_tmp,"slash")." WHERE id_fichier='".intval($id_fichier_tmp)."'");
			}
		}
		...
		db_maj_champ_ajoute("2.13.0", "gt_utilisateur_inscription", "message", "ALTER TABLE gt_utilisateur_inscription ADD message TEXT AFTER pass");
		...
		////	V2.13.2 : SUPPR LES CONSTANTES OBSOLETES DANS LE FICHIER DE CONFIG
		...
		////
		////	Modifier la date de dernière mise à jour  &  numero de version (VERSION_MAJ_DATE UP)
		////	Modifier si besoin les fichiers .sql  "install"  +  "test"  !!!
		////	Tester ancienne mise à jour avec  BDD_TEST_01-2010.sql  (import en latin1 + config.inc.php avec "version=2.8.2")
