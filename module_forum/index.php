...
elements_width_height_type_affichage("100%","300px","liste");
		...
		if($affiche_liste_themes==true)
		{
			foreach($themes_sujets_bis as $theme_tmp)
			{
				...
				if($theme_tmp["id_theme"]>0 || count($sujets_tmp)>0)
				{
					...
					if(count($sujets_tmp)>0)	$tab_nb_sujets_messages .= "<tr><td style='width:90px'>".count($sujets_tmp)." ".(count($sujets_tmp)>1?$trad["FORUM_sujets"]:$trad["FORUM_sujet"])."</td><td>".$trad["FORUM_dernier_message"]." ".auteur($sujets_tmp[0]["id_utilisateur"],$sujets_tmp[0]["invite"]).", &nbsp;".temps($sujets_tmp[0]["date_crea"])."</td></tr>";
					...
					if(count($messages_tmp)>0)		$tab_nb_sujets_messages .= "<tr><td style='width:90px'>".count($messages_tmp)." ".(count($messages_tmp)>1?$trad["FORUM_messages"]:$trad["FORUM_message"])."</td><td>".$trad["FORUM_dernier_message"]." ".auteur($messages_tmp[0]["id_utilisateur"],$messages_tmp[0]["invite"]).", &nbsp;".temps($messages_tmp[0]["date_crea"])."</td></tr>";
					if($tab_nb_sujets_messages!="" || $tab_nb_sujets_messages!="")	$tab_nb_sujets_messages = "<table style='width:100%' cellpadding='0px'>".$tab_nb_sujets_messages."</table>";
					////	AFFICHAGE
					echo "<div class='div_elem_deselect' style='".STYLE_ELEMENT_LISTE."height:55px;padding:3px;margin-bottom:10px;line-height:16px;'>";
						echo "<table class='div_elem_table lien' cellpadding='5px' onClick=\"redir('".php_self()."?theme=".$theme_tmp["id_theme"]."');\"><tr>";
							...
							echo "<td style='vertical-align:middle;width:400px;'>".$tab_nb_sujets_messages."</td>";
						echo "</tr></table>";
					echo"</div>";
				}
			}
			...
			foreach(tableau_elements_page($sujets_total) as $sujet_tmp)
			{
				...
				require PATH_INC."element_menu.inc.php";
					echo "<div class='div_elem_contenu'>";
						echo "<table class='div_elem_table lien' cellpadding='0px' cellspacing='0px' ".infobulle($trad["FORUM_voir_sujet"])."><tr>";
							////	TITRE / DESCRIPTION (début)
							if($sujet_tmp["titre"]=="")		$sujet_tmp["titre"] = strip_tags(text_reduit($sujet_tmp["description"]));
							$style_pas_consult =  (is_dernier_message_consulte($sujet_tmp["users_consult_dernier_message"])==false)  ?  "style='text-decoration:underline;'"  :  "";
							echo "<td style='vertical-align:middle;'><span ".$style_pas_consult.">".$sujet_tmp["titre"]."</span></td>";
							echo "<td style='vertical-align:middle;text-align:right;width:400px;'>";
								////	AUTEUR / DATE
								$auteur_tmp = user_infos($sujet_tmp["id_utilisateur"]);
								echo "<div style='margin-bottom:7px;'>".auteur($auteur_tmp,$sujet_tmp["invite"]).", &nbsp;".temps($sujet_tmp["date_crea"])."</div>";
								////	NB DE MESSAGES
								$nbmessages = db_valeur("SELECT count(*) FROM gt_forum_message WHERE id_sujet=".$sujet_tmp["id_sujet"]." ");
								$style_message	= ($nbmessages>0)  ?  STYLE_SELECT_YELLOW  :  "";
								$txt_message	= ($nbmessages>1)  ?  $trad["FORUM_messages"]  :  $trad["FORUM_message"];
								echo "<span style='margin-top:10px;".$style_message."'>".$nbmessages." ".$txt_message."</span>";
								////	DERNIER POST ?
							...
							echo "</td>";
							////	PHOTO
							echo "<td style='width:60px;padding-left:5px;text-align:right;'>".photo_user($auteur_tmp,55)."</td>";
						echo "</tr></table>";
					echo "</div>";
				echo "</div>";
			}
